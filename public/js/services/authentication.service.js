(function () {
    ...
    function AuthenticationService($http, $cookies, $rootScope, $timeout) {
        ...
        var timeoutResponse = {
            data: {
                success: false,
                message: 'Sorry, there was a timeout.  Please try again later!'
            }
        }
        ...
        function Login(user, callback) {
            ...
            $http.post(localHostAddress + '/users/login', user, config).then(function (response) {
                callback(response.data);
            }, function () {
                if (response == null) {
                    var response = timeoutResponse
                }
                callback(response.data);
            });
